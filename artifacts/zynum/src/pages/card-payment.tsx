...
import { ChevronLeft, CreditCard, CheckCircle2, AlertCircle, Loader2, ExternalLink, QrCode, Lock } from "lucide-react";
...
export default function CardPaymentPage() {
  ...
  return (
    <div className="w-full bg-white" style={{ minHeight: "100dvh" }}>
      <AnimatePresence mode="wait">

        {/* ── AMOUNT STEP ─────────────────────────────────────────────── */}
        {step === "amount" && (
          <motion.div key="amount" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="flex flex-col" style={{ minHeight: "100dvh" }}>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-32" style={{ paddingTop: HEADER_H + 16 }}>
                  ...
                  return (
                    <button key={a}
                      ...
                      className={`py-3 rounded-2xl text-sm font-bold transition-all ${
                        amountFcfa === a ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"
                      }`}
                    >
                      {label}
                    </button>
              ...
              {/* Security badge */}
              <div className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-green-50 border border-green-100">
                <Lock className="w-4 h-4 text-green-600" />
                <p className="text-xs text-green-700 font-semibold">Paiement sécurisé — données chiffrées SSL</p>
              </div>
            </div>

            {/* Fixed footer */}
            <div className="fixed bottom-0 left-0 right-0 px-4 pb-8 pt-4 border-t border-gray-100 bg-white">
              <button
                onClick={() => {
        ...
        {/* ── CARD FORM STEP ──────────────────────────────────────────── */}
        {step === "card" && (
          <motion.div key="card" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            ...
            <div className="flex-1 overflow-y-auto px-5 pb-32" style={{ paddingTop: HEADER_H }}>
              ...
              <div className="pt-6 pb-5">
                <div className="w-full h-44 rounded-3xl p-5 text-white relative overflow-hidden"
                  style={{ background: "linear-gradient(135deg, #1A3FFF 0%, #4F46E5 100%)" }}>
                  ...
                  <div className="flex justify-between items-start mb-4 relative">
                    ...
                    <div className="text-right">
                      <p className="text-[10px] uppercase tracking-wider text-white/80">Montant</p>
                      <p className="font-black text-base text-white">{amountFcfa.toLocaleString("fr-FR")} FCFA</p>
                    </div>
                  </div>
                  <p className="font-mono text-base tracking-[0.2em] mb-4 relative text-white">
                    {cardNumber || "•••• •••• •••• ••••"}
                  </p>
                  <div className="flex justify-between items-end relative">
                    <div>
                      <p className="text-[10px] uppercase tracking-wider mb-0.5 text-white/80">Titulaire</p>
                      <p className="font-bold text-sm uppercase text-white">{holderName || "VOTRE NOM"}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] uppercase tracking-wider mb-0.5 text-white/80">Expire</p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-green-50 border border-green-100">
                  <Lock className="w-4 h-4 text-green-600" />
                  <p className="text-xs text-green-700 font-semibold">Paiement sécurisé — données chiffrées</p>
                </div>
              </div>
            </div>
        ...
        {/* ── PROCESSING STEP ─────────────────────────────────────────── */}
        {step === "processing" && (
          <motion.div key="processing" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
            transition={{ type: "tween", duration: 0.22 }}
            className="flex flex-col" style={{ minHeight: "100dvh" }}>
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed">
                      ...
                      <span className="font-bold text-gray-800">{amountFcfa.toLocaleString("fr-FR")} FCFA</span>.
                    </p>
                  </div>
                  {paymentUrl && (
